...
if($config->comments && $config->page_type === 'article') {
    ...
    Weapon::add('shell_after', function() use($config) {
        if( ! $path = Asset::loaded('manager/assets/shell/editor.css')) {
            echo Asset::stylesheet($path);
        }
        if( ! $path = Asset::loaded($config->protocol . ICON_LIBRARY_PATH)) {
            echo Asset::stylesheet($path);
        }
    });

    Weapon::add('SHIPMENT_REGION_BOTTOM', function() use($config) {
        if( ! $path = Asset::loaded('manager/assets/sword/editor/editor.min.js')) {
            echo Asset::javascript($path);
        }
        if( ! $path = Asset::loaded('manager/assets/sword/editor/mte.min.js')) {
            echo Asset::javascript($path);
        }
        if( ! $path = Asset::loaded('manager/assets/sword/editor/hte.min.js')) {
            echo Asset::javascript($path);
        }
